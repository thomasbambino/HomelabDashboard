    ...
    <>
      <Card className={`backdrop-blur-sm bg-background/95 ${server.background ? `bg-[url('${server.background}')] bg-cover` : ""}`}>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <div className="flex items-center gap-2">
            {user?.role === "admin" && (
              <Button variant="ghost" size="sm" onClick={() => setShowEditDialog(true)} className="px-2">
        ...
        <CardContent>
          <div className="space-y-2">
            ...
            <div className="flex items-center justify-between gap-2 mt-4 pt-2 border-t">
              ...
              {/* Server Address */}
              {server.port && (
                <Button variant="ghost" size="sm" className="text-xs" onClick={() => copyServerAddress(server.port)}>
      ...
      <EditGameServerDialog server={server} open={showEditDialog} onOpenChange={setShowEditDialog} />
    </>
  );
}